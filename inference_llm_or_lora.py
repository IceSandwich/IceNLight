...
class Args:
	checkpoint_or_lora_path: str

def SetupArguments() -> Args:
	args = argparse.ArgumentParser()
	args.add_argument("--checkpoint_or_lora_path", type=str, required=True)
...
def main(args: Args):
	model, tokenizer = FastLanguageModel.from_pretrained(
		model_name = args.checkpoint_or_lora_path,
		max_seq_length = 2048,
		load_in_4bit = True,
		device_map="auto",
	)
	...
	print("- Type `/bye` to exit. Type `/system xxx` to set system prompt. Type `/clear` to clean history.")
	messages = []
	while True:
		input_str = input(">>> ")
